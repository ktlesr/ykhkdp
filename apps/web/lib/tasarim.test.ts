...
test("hero istisnası TEK YÜZEYE kapsanmış: glow başka yerde yok", async () => {
  /**
   * Handoff §1.8 glow'u ve CAM EFEKTİNİ yasaklıyor. Tanıtım hero'su için
   * gevşetildi ama gevşeme sızmamalı: `filter`, `backdrop-filter`,
   * `drop-shadow` ve `box-shadow` yalnızca `.hero-*` seçicilerinde durabilir.
   *
   * `backdrop-filter` ayrıca yazılıyor çünkü `filter` kalıbı onu YAKALAMAZ:
   * önündeki karakter `-`, kalıbın istediği boşluk/`;`/satır başı değil.
   * Kapsam testinin sessizce geçmesi, kuralın olmamasından beterdir.
   *
  ...
  for (const kural of css.split("}")) {
    const [secici, govde = ""] = kural.split("{");
    if (!/(^|[\s;])(backdrop-filter|filter|box-shadow)\s*:|drop-shadow\(/.test(govde)) continue;
...
test("handoff §1.8 yasakları: gölge, gradyan, 3px üstü köşe yok", async () => {
  ...
  // (handoff §2 doku utility'leri bu biçimde tanımlı). Hero için açılmış
  // gradyan istisnası KALDIRILDI: maske yerine buzlu plaka kullanılıyor,
  // gradyana ihtiyaç kalmadı. Kullanılmayan istisna kapatılır.
